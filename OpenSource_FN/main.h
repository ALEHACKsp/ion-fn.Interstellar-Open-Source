	...
	try
	{
		...
		for (PVOID enemy : players)
		{
			...
			if (settings::boxesp)
			{
				...
				window.DrawList->AddRect(ImVec2(temp.x, temp.y), ImVec2(temp1.x, Feet.y), ImGui::GetColorU32({ 0.79f, 0.19f, 0.65f, 1.00f }), 0, 0, 0.5);
			}

			//draws line from player head to bottom-center of the screen
			if (settings::snaplines)
				window.DrawList->AddLine(ImVec2(Head.x, Head.y), ImVec2(X / 2, Y), ImGui::GetColorU32({ 0.79f, 0.19f, 0.65f, 1.00f }));
